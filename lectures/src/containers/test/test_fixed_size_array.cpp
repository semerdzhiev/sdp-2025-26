...
#include "containers/fixed_size_array.h"

using dsa::fixed_size_array;
...
template <typename T>
void checkWhetherEmpty(fixed_size_array<T>& arr)
{
  const fixed_size_array<T>& cref = arr;
  CHECK(arr.size() == 0);
...
template <typename T>
void checkWhetherNotEmpty(fixed_size_array<T>& arr, size_t expectedSize)
{
  const fixed_size_array<int>& cref = arr;
...
SCENARIO("fixed_size_array() creates an empty object", "[fixed_size_array]")
{
  GIVEN("A default-constructed fixed_size_array object")
  {
    fixed_size_array<int> arr;
...
SCENARIO("fixed_size_array(size_t N = 0) constructs an empty object", "[fixed_size_array]")
{
  GIVEN("A fixed_size_array object created with zero size")
  {
    fixed_size_array<int> arr(0);
...
SCENARIO("fixed_size_array(size_t N > 0) correctly constructs an object with the specified size", "[fixed_size_array]")
{
  GIVEN("A fixed_size_array object created with positive size")
  {
    fixed_size_array<int> arr(5);
...
SCENARIO("fixed_size_array(size_t N > 0) throws when passed a size that does not fit in memory", "[fixed_size_array]")
{
  GIVEN("A size, which does not fit in the heap")
  {
    ...
    WHEN("We call fixed_size_array(size_t) with that size") {
      THEN("a std::bad_alloc exception is thrown") {
	      REQUIRE_THROWS_AS(fixed_size_array<int>(size), std::bad_alloc);
      }
    }
  }
}

SCENARIO("fixed_size_array(const fixed_size_array&) throws when passed a size that does not fit in memory", "[fixed_size_array]")
{
...
SCENARIO("fixed_size_array::operator[] can be used to access all elements of the array", "[fixed_size_array]")
{
  GIVEN("An array with a specific size N")
  {
    size_t N = 10;
    fixed_size_array<size_t> arr(N);
    ...
    WHEN("We fill the array with exactly N elements using operator[]")
    {
      ...
      {
        const fixed_size_array<size_t>& cref = arr;
        for (size_t i = 0; i < N; ++i)
          REQUIRE(cref[i] == i);
      }
    }
...
SCENARIO("fixed_size_array::at() can be used to access all elements of the array", "[fixed_size_array]")
{
  GIVEN("An array with a specific size N")
  {
    size_t N = 10;
...
SCENARIO("fixed_size_array::at() throws if the index is not valid", "[fixed_size_array]")
{
  GIVEN("A non-empty array")
  {
    fixed_size_array<size_t> arr(5);
...
public:
  ...
  fixed_size_array<size_t> arr = fixed_size_array<size_t>(initialSize);
  const fixed_size_array<size_t>& cref = arr; /// Constant reference to the array
...
SCENARIO("fixed_size_array::operator== returns true when comparing two empty arrays")
{
  GIVEN("Two empty arrays")
  {
    fixed_size_array<size_t> arr1;
    fixed_size_array<size_t> arr2;
...
SCENARIO("fixed_size_array::operator== returns true when the contents of two arrays match")
{
...
SCENARIO("fixed_size_array::operator== returns false when two arrays of the same size have different contents")
{
...
SCENARIO("fixed_size_array::operator== returns false when we compare non-empty arrays of different sizes")
{
...
SCENARIO("fixed_size_array::operator== returns false when we compare an empty array to a non-empty one")
{
  GIVEN("An empty and a non-empty array")
  {
    ArrayOfNumbersFixture nonEmptyFx;
    fixed_size_array<size_t> empty;
...
SCENARIO("fixed_size_array::fill_from() does not change the array when copying from an empty one", "[fixed_size_array]")
{
  GIVEN("A non-empty array")
  {
...
SCENARIO("fixed_size_array::fill_from() partially copies the contents of a larger array", "[fixed_size_array]")
{
  GIVEN("A non-empty array")
  {
    ...
    WHEN("We we try to fill from an array with a greater size and different elements") {
      ...
      fxShort.arr.fill_from(fxLong.arr);
...
SCENARIO("fixed_size_array::fill_from() copies the element of a shorter array without overwriting the rest of the original contents", "[fixed_size_array]")
{
  GIVEN("A non-empty array")
  {
    ...
    WHEN("We we try to fill from an array with a shorter size and different elements") {
      ...
      fxLong.arr.fill_from(fxShort.arr);
...
SCENARIO("fixed_size_array: The contents of two arrays can be swapped", "[fixed_size_array]")
{
...
SCENARIO("fixed_size_array(const fixed_size_array&) creates an identical copy of an empty array", "[fixed_size_array]")
{
  GIVEN("An empty array")
  {
    fixed_size_array<int> empty;

    WHEN("We pass it to the copy constructor")
    {
      fixed_size_array<int> copy(empty);
...
SCENARIO("fixed_size_array(const fixed_size_array&) creates an identical copy of an non-empty array", "[fixed_size_array]")
{
  GIVEN("A non-empty array")
  {
    ArrayOfNumbersFixture fx;
    ...
    WHEN("We pass it to the copy constructor")
    {
      fixed_size_array<size_t> copy(fx.arr);
...
SCENARIO("FixesSizeArray::operator= empties an array when assigning from the empty array", "[fixed_size_array]")
{
  GIVEN("An empty array")
  {
    fixed_size_array<int> empty;
...
SCENARIO("FixesSizeArray::operator= correctly copies a non-empty array", "[fixed_size_array]")
{
  GIVEN("A non-empty array")
  {
...
SCENARIO("fixed_size_array(fixed_size_array&&) correctly moves the contents of another array", "[fixed_size_array]")
{
  GIVEN("A non-empty array with a buffer at a given address")
  {
    ...
    {
      fixed_size_array<size_t> movedTo(std::move(fx.arr));
      const fixed_size_array<size_t>& cref = movedTo;
...
SCENARIO("fixed_size_array::operator=(fixed_size_array&&) correctly moves to an empty array", "[fixed_size_array]")
{
  GIVEN("A non-empty source array with a buffer at a given address")
  {
    ...
    {
      fixed_size_array<size_t> movedTo;
...
SCENARIO("fixed_size_array::operator=(fixed_size_array&&) correctly moves to a non-empty array", "[fixed_size_array]")
{
  GIVEN("A non-empty source array with a buffer at a given address")
  {
    ArrayOfNumbersFixture fx;
    ...
    {
      fixed_size_array<size_t> movedTo(10);
      movedTo = std::move(fx.arr);
      const fixed_size_array<size_t>& cref = movedTo;
